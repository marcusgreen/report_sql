...
### Actionable reports (bulk row-select + built-in ops)

A published query can be made **actionable**: its rows get a leading select-all checkbox column and a bulk-action bar that runs a built-in Moodle operation (enrol / unenrol / suspend / message / add-to-cohort) over the ticked rows. Config lives in the `actionsmeta` JSON column (`{enabled, ops[], subject, subjectcolumn, params}`), read **live** (not frozen at publish) via `query::actions_meta()` / `actions_enabled()` / `action_ops()` / `action_subjectcolumn()` / `action_params()`.

- **Why a plugin-owned report.** The data report is a core RB *datasource* report (`/reportbuilder/view.php`), which has **no row-selection UI**. Only a `system_report` supports `set_checkbox_toggleall()`. So the actionable table is a separate `classes/reportbuilder/local/systemreports/query_actions.php` over the same VIEW (reusing the `adhoc_view` entity + `columnsmeta`), rendered on `actions.php`. It exposes the subject id as a base field (`… AS subjectid`), and re-applies the data report's per-viewer scoping via `query::viewer_scope_sql($alias)` → `add_base_condition_sql()` so it can never surface a row the data report hides. `can_view()` double-gates: `report/sql:actexecute` **AND** `query::current_user_can_view_report()`. Checkboxes render only when created with the `withcheckboxes` parameter (host page passes it), mirroring core `admin/user.php`.
- **Action framework** (`classes/local/action/`). `action_handler` interface + abstract `base_action` implement the execute loop **once**: per subject → `target_context()` → `has_capability(required_capability())` in that context (skip + reason if denied — the **second** gate, on top of `actexecute`) → `apply_one()` with per-subject error isolation. So no handler can forget the cap check. Each handler wraps one core API: `enrol_user`/`unenrol_user` (shared `manual_enrol_action` base, manual enrol instance, course from param or report scope), `suspend_user` (`user_update_user` + kill sessions; refuses siteadmin/self), `message_user` (`message_send` via the `actionmessage` provider in `db/messages.php`), `cohort_add` (`cohort_add_member`, idempotent). `action_registry` is the single source of truth (`all`/`instance`/`for_ops`/`menu`).
- **Edit form + persistence.** `edit_query_form::add_actions_elements()` (built in `definition_after_data`, column-dependent like the filter/chart sections) offers enable / subject / subject-column / multi-select ops / per-op params; `validation()` enforces coherent config; `query::build_actionsmeta()` validates ops against the registry and the subject column against `columnsmeta`, wired into `save()`'s published branch. Unpublished queries show a "publish first" note.
- **Dispatch.** The action bar (`bulk_action_form`, its op list limited to the query's enabled ops) posts to `action_apply.php`: `require_sesskey`, op re-validated against `action_ops()`, same double gate, `subjectids` normalised, row cap `REPORT_SQL_ACTION_MAXROWS` (500). Destructive ops (`is_destructive()`) get an `$OUTPUT->confirm` interstitial (continue via a sesskey'd GET) before anything changes. Then `handler->execute($ids, $ctx, action_params())`, an `action_applied` event at the report context, and a redirect back to `actions.php` with an *N applied / M skipped* notification.
- **JS.** `amd/src/bulk_actions.js` (adapted from `core_admin/bulk_user_actions`): the checkbox togglegroup is core's `report-select-all`; on change it writes the checked subject ids into the form's hidden `subjectids` and disables **Apply** until ≥1 row is selected, rebinding on RB dynamic-table refresh.
- **UI entry.** The `queries` system report's **Open actions** kebab (`i/checkedcircle`) links to `actions.php?id=`, shown only for a published, actions-enabled query to holders of `actexecute` in the query's own context (`courseid, actionsmeta` are in its base fields).
- **Portability.** `transfer` carries `actionsmeta` but **strips `params`** (roleid/courseid/cohortid are local ids); imports land as drafts needing re-config, like all imports.

The capability `report/sql:actexecute` (system, `RISK_DATALOSS | RISK_SPAM`, manager) admits a user to the actions UI only; the real authority for each mutation is the op's own core capability, checked per target context. Caveat: `RB tabular export` is irrelevant here — the actionable table is an on-screen/interactive surface, not a scheduled artifact.
...
- `columnsmeta` (JSON, frozen at publish), `chartmeta` (JSON chart config), `audiencemeta` (JSON audience picker choice — see [Report visibility](#report-visibility-who-can-open-the-report)), `actionsmeta` (JSON actionable-report config, read live — see [Actionable reports](#actionable-reports-bulk-row-select--built-in-ops))
- `courseid` (0 = site-wide), `visible`
- `useridcolumn`, `coursecolumn`, `pagecoursecolumn` — per-user / per-course filter column choices
- `timecreated`, `timemodified`

Auditing is done through Moodle's standard event log (`logstore_standard_log`), not a custom table. `classes/event/` defines five query-lifecycle events (`query_created`, `query_updated`, `query_published`, `query_unpublished`, `query_deleted`) plus `action_applied` (bulk action run — raised at the **report** context with `other = {name, op, applied, skipped}`), all extending `query_event_base` (→ `\core\event\base`). The lifecycle events are raised at `context_system` with `objectid` = query id and the query name in `other['name']` (so delete descriptions still render after the record is gone), and are triggered from `classes/local/query.php`. Viewable at **Site admin → Reports → Logs**.
...
| `actexecute` | system | Run bulk actions on report rows (see [Actionable reports](#actionable-reports-bulk-row-select--built-in-ops)) |
