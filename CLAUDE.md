...
- Rejects unknown `%%…%%` tokens via `is_supported_token()`. Supported tokens (`%%WWWROOT%%`, `%%COURSEID%%`, `%%COURSECONTEXT%%`, `%%NOW%%`, `%%CONTEXT_*%%` level constants via `view::context_level_tokens()`, `%%TIMESTAMP(expr[, format])%%`, `%%EPOCH(datetime)%%`, `%%CASE(expr, mode)%%`, `%%GROUP_CONCAT([DISTINCT ]expr[, 'sep'])%%`, `%%VIEWER(expr)%%`, `%%TEACHES(expr)%%`, `%%PAGECOURSE(expr)%%`) are exempt because they are substituted later in `view::resolve_placeholders()`, not at validate time. The literal `%%USERID%%` (and `USER_ID`/`USERIDS` spellings) stays **rejected** with a bespoke message (`errplaceholderuserid`) steering authors to `%%VIEWER()%%` / the Per-user filter — a fixed shared view cannot carry a per-request user id.
...
`%%GROUP_CONCAT([DISTINCT ]expr[, 'sep'])%%` resolves in the same pass to the live DB family's string-aggregate spelling — MySQL/MariaDB `GROUP_CONCAT([DISTINCT ]expr SEPARATOR 'sep')`, Postgres `string_agg([DISTINCT ](expr)::text, 'sep')` — so a query grouping rows into one delimited string is portable across engines instead of hard-coding `group_concat`/`string_agg`. Unlike `%%CASE%%`/`%%TIMESTAMP%%` this is a **pure SQL rewrite** (no `columnsmeta`, no display callback): the aggregated column introspects as plain `text`. The separator is an optional single-quoted literal (may contain a comma; defaults to `','` to match MySQL's native default); an optional leading `DISTINCT` sits in the same position for both engines; `expr` cannot contain `%`. Postgres `string_agg` requires text input, so `expr` is cast `::text` (harmless on already-text columns); MySQL coerces implicitly. Use inside a query with a `GROUP BY`.

The **per-viewer filter tokens** `%%VIEWER(expr)%%`, `%%TEACHES(expr)%%`, `%%PAGECOURSE(expr)%%` (`view::FILTER_TOKENS`) all resolve in the same pass to the **bare column expression** `(expr)` — an ordinary output column, so the view itself stays **unfiltered**. Unlike `%%CASE%%`/`%%TIMESTAMP%%` they drive no display callback and no `columnsmeta`: they are the **inline form of the three per-viewer filter dropdowns**, marking a select-list column that a run-time row filter keys on. At publish, `query::resolve_marked_column($columns, $sql, $token)` recovers the produced column (keyed the same way — `AS` alias, else trailing identifier, via `view::filter_token_columns($sql, $token)`) and adopts it onto the record: `VIEWER`→`useridcolumn`, `TEACHES`→`coursecolumn`, `PAGECOURSE`→`pagecoursecolumn`. The existing runtime filters then apply with no dropdown choice — `adhoc_query` init (userid `= $USER->id` + column hidden; course `IN` taught-courses, column stays visible) and `fetch_rows_for_viewer()` (all three, incl. page-course which only bites in the block/filter embed — the RB standalone viewer has no current course). A token **overrides** the matching form-chosen column. It is a **hard publish error** (`err<token>ambiguous` / `err<token>unresolved`, token lower-cased) to have more than one token of a kind or one that names no output column — the intended row scope would silently not apply, so `resolve_marked_column()` rejects rather than drop it (contrast `%%LINK%%`, which degrades to plain text). `view::filter_token_count($sql, $token)` vs `filter_token_columns()` size is how "no token" is told from "unnameable token". `expr` cannot contain `%`.
...
**Advisory analysis** (`classes/external/test_query.php` → `analyser::analyse()` in `classes/local/sql/analyser.php`): the edit form's **Test query** button (`amd/src/test.js`) runs the SQL without saving and returns `{ok, error, rowcount, datecolumns, suggestions, warnings, indexinfo, columnindex}` — advisory only, never a publish gate. After static validation, `analyse()` runs a live `dry_run()` (`SELECT * FROM (<sql>) rs_dryrun LIMIT 1`) before the advisory probes: a query that will not execute — syntax error, missing table/column, row-dependent select-list error — sets `ok=false` with the cleaned DB message (`validator::clean_error()`) and returns early, so those surface to the author instead of being swallowed as `rowcount=-1`. The remaining probes (row count, date columns, index/scan hints) still degrade silently on error. This mirrors the publish-gate live check in `external/validate_sql.php`, except Test query never issues a `CREATE VIEW`, so duplicate-output-column errors stay a publish-time discovery. `test.js` renders each `datecolumns` entry as a **click-to-wrap** control: `maskSql()`/`selectListRegion()`/`splitItems()`/`wrapTimestamp()` rewrite that column's select-list item in place, wrapping its expression in `%%TIMESTAMP(...)%%` (idempotent; returns null for `SELECT *`; a leading `DISTINCT` on the first item is kept outside the token).

`analyse()` takes an optional third arg `?string $viewname`. When a caller has **already built and executed** a live view for the exact SQL, it passes that view name and `analyse()` skips both its own `dry_run()` (the caller proved the SQL runs) and the throwaway `_chk` probe view in `date_columns()` (it introspects the supplied view instead). The **inline Preview** is the caller: `report_sql_output_fragment_preview()` builds its per-user preview view, runs `analyser::analyse($sql, $courseid, $viewname)` **once**, and reuses that single `$feedback` for two things — `report_sql_preview_summary($feedback)` renders a row-count + performance-warning strip **above** the rendered table (a preview doubles as a lightweight test off a single view build), and the `preview` system report (`classes/reportbuilder/local/systemreports/preview.php`) is passed `$feedback['columnindex']` as a `columnindex` JSON parameter to badge column headers (see below). Advisory only: a failed analysis pass degrades to just the rendered rows, and a failed/absent columnindex degrades to plain headers. Preview reuses the `checkrowcount` string; the click-to-wrap date UX stays exclusive to the Test button (the editor is not in scope in the read-only preview).

**Per-column index status** (`analyser::column_index_status()` + `indexed_columns_all()`): for every top-level select-list item that is a **bare source-table column reference** (optionally `{table}.`/alias-qualified, optionally `AS`-aliased — the same item-splitting as `case_columns()`), the analyser looks up whether that source column appears in *any* position of *any* index on its base table (or is the primary key) and returns `columnindex: array<{col, indexed}>` keyed by the *output* name (alias if present). Expression columns (functions, `%%TOKEN%%`s, computed values) are omitted entirely — "indexed" has no meaning for them and reporting them as "not indexed" would misleadingly suggest a fixable gap. This is deliberately broader than `indexed_leading_columns()` (used by the ORDER BY sort hint in `index_report()`, which needs a *leading* index column to actually satisfy a sort) — a non-leading indexed column still helps an equality lookup, so it is still worth flagging.

The **inline Preview** visualises this per-column rather than as a summary line: `preview::initialise()` decodes the `columnindex` parameter and, for each RB column whose name matches an **indexed** entry, appends a small green Font Awesome bolt icon (`fa-bolt`, tooltip `indexedcolumn` string) to that column's header, routed through the existing `reportsourceheader = '{$a}'` passthrough lang string (see `adhoc_view::raw_title()`) rather than a new per-badge lang string. **Not-indexed columns are left unbadged on purpose** — most columns in a typical report are unindexed, so marking the exception (indexed) rather than the common case keeps the header row readable. `report_sql_preview_summary()` deliberately does **not** also print a `checkindexedcolumns` / `checknotindexedcolumns` text line — that would just repeat what the header badges already show. The **Test-query panel** (`test.js`, no table headers to badge) is the only surface that prints those text lines, via `appendColumnIndexAlert()`.

### Cross-DB portability (MySQL/MariaDB ⇄ Postgres) & MySQL→Postgres breakage

Every dialect difference the plugin *owns* is routed through `$DB->get_dbfamily()` so one saved query runs unchanged on both engines. The portable-token mechanisms: `%%NOW%%` / `%%EPOCH%%` / `%%GROUP_CONCAT%%` (engine-specific spelling emitted at `resolve_placeholders()` time), `%%TIMESTAMP%%` / `%%CASE%%` (bare portable expr in SQL + PHP display callback — the only cross-engine path for `title`/`sentence`, which have no portable SQL), `query::map_db_type()` typing from engine-agnostic `$DB->get_columns()`, all bundled samples using tokens not dialect date functions, and the legacy importers' DB-family-aware `rewrite_date_functions()`. `view::columns()` has a Postgres branch (`pg_view_columns()`) for view-column introspection; MySQL keys are lower-cased to keep RB's alias lookup in sync. Full reference: [docs/cross-db.md](docs/cross-db.md).

**What the plugin does *not* shield.** Hand-written SQL can use MySQL-only syntax that passes static validation, runs on MySQL, and fails (or silently misbehaves) on Postgres. The trap: author + validate on a MySQL site, export, import on a Postgres site — it breaks only at publish there. The live check (`validate_sql.php`) and Test-query (`analyser::analyse()`) run a real `dry_run()` + `CREATE VIEW`, so they **catch every hard error below — but only against a Postgres connection**. Only the MySQL date functions get an early PG-specific *warning* (`validator::MYSQL_DATE_FUNCTIONS`, checked when `$dbtype === 'pgsql'`); everything else surfaces as a raw DB error at publish on PG.

- **Hard errors on Postgres** (query won't execute): loose `GROUP BY` (non-aggregated column not in `GROUP BY`); implicit coercion (`varchar = 3`, `text + int`, string↔date compares); boolean context (`WHERE intcol` / `WHERE 1`); `IFNULL`→`COALESCE`, `IF()`→`CASE WHEN`; native `GROUP_CONCAT`→the token; native date funcs (`DATE_FORMAT`/`FROM_UNIXTIME`/`STR_TO_DATE`/`DATEDIFF`/`DATE_ADD/SUB`/`UNIX_TIMESTAMP(arg)`) → tokens (**warned**); double-quote string literal (`= "foo"` reads as an identifier); backtick identifiers; `LIMIT x, y`→`LIMIT y OFFSET x`; `CAST(x AS UNSIGNED)`; `RAND()`/`CURDATE()`/`YEAR()`/`SUBSTRING_INDEX`.
- **Silent-wrong** (runs on both, different answer — worst kind): `5/2` (MySQL `2.5` float vs PG `2` int-trunc); `||` (MySQL logical-OR vs PG concat); `LIKE` (MySQL case-insensitive vs PG case-sensitive — use `ILIKE`/`LOWER()`); `DIV` (MySQL int-div, PG error).
- **Top three real risks** for typical Moodle report SQL: loose `GROUP BY`, implicit type coercion, boolean context — none caught by static validation, all fail only at PG runtime (MySQL 5.7+ default `ONLY_FULL_GROUP_BY` makes the first rarer on new installs, still common in legacy/imported SQL).
...
**Third-party plugin dependencies.** A source may declare `"requires"` — a frankenstyle component name or array of them (e.g. `"requires": "mod_attendance"`) — for a query over a plugin's own tables. **`export()` fills this automatically**: `transfer::detect_requires($sql)` extracts the query's `{table}` refs (`validator::braced_tables()` — string/comment-masked, brace-only) and maps each to the installed **non-standard** plugin that owns it via `thirdparty_table_map()` (reads each third-party plugin's `db/install.xml` `<TABLE NAME>`s, per-request cached; core/standard tables are always present so never emitted). Because export runs on a site where those plugins are installed, the tables resolve and the dependency is baked in **certainly and named** — no table→plugin guessing on the importing site. Omitted from the export JSON when the query is core-only. Hand-maintained `samples.json` can still set `requires` by hand (it is not produced through `export()`). `transfer::parse()` normalises it (`PARAM_COMPONENT`, blanks/dupes dropped) onto the source. `transfer::component_available()` (plugin-manager `get_plugin_info()` + `is_installed_and_upgraded()`; bare `core` always true, unknown → false) is the install test. `bundled_samples(bool $includeunavailable = false)` by default **drops** any source whose required plugin is missing (keys preserved — no reindex — so each survivor's `index` still equals its array key, which the import selector relies on) and annotates each with `available`, `duplicate` and `requiresmeta` (`[component, name, thirdparty, installed]` per dep; the name falls back to the raw component when the plugin — hence its lang pack — is absent). `import()` also **refuses** a source with an unmet requirement (belt-and-braces: stale posted index / uploaded JSON), folding it into `skipped[]` via `samples:requiresmissing`. `samples.php` badges only the **third-party** deps (`is_standard()` false) — a standard/core dependency is always present and needs no note.

**Show-all reveal (samples.php only).** `count_samples()` / `import_samples()` / the import POST handler use the default available-only set, so a missing-plugin sample is **never importable**. The *display* path calls `bundled_samples(true)` (superset, unavailable ones flagged `available = false`) and, when at least one is unavailable, offers a **Show all** checkbox (`samples:showall`, carrying the hidden count). It is a **sibling GET form** (never nested in the POST import form) that `amd/src/samples.js` submits on change to flip `?showall`. Revealed rows render **disabled** (can't import — tables absent) with a **warning** badge `samples:requiresmissingbadge` ("Requires mod_x (not installed)"). `import.php` (uploaded files) has **no** such toggle — it hard-hides unmet rows, since an uploaded file has no onboarding value. Absent/empty `requires` = core-only, always shown.
