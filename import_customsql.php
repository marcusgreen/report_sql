 ...
 * {@see \report_sql\local\customsql_import}) and rejected (with a reason). The admin ticks
...
admin_externalpage_setup('report_sql_importcustomsql');

$indexurl = new moodle_url('/report/sql/index.php');
$pageurl = new moodle_url('/report/sql/import_customsql.php');

echo $OUTPUT->header();
echo $OUTPUT->heading(get_string('customsqlimport:title', 'report_sql'));

if (!customsql_import::available()) {
    echo $OUTPUT->notification(
        get_string('customsqlimport:unavailable', 'report_sql'),
...
echo html_writer::tag('p', get_string('customsqlimport:intro', 'report_sql'));

if (empty($importable)) {
    echo $OUTPUT->notification(
        get_string('customsqlimport:noneimportable', 'report_sql'),
